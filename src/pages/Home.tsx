...
import { FaMagic, FaShieldAlt, FaCopy, FaCheck, FaPlus, FaBriefcase, FaChevronRight, FaGlobe, FaUserAlt, FaHistory, FaClock } from 'react-icons/fa'
...
export default function Home() {
  ...
  const { data: projects = [], isLoading: projectsLoading } = useGetProjectsQuery(undefined, { skip: !token })
  
  const { data: historyResponse, isFetching: historyLoading } = useGetMessagesQuery(
  ...
  const [moderateMessage, { isLoading: moderating }] = useModerateMessageMutation()
  ...
  const handleCreateProject = async () => {
    ...
    try {
      ...
      setProjectForm({ name: '', description: '', clientName: '', platform: 'FIVERR', orderLink: '' })
      ...
      <aside className="w-80 border-r border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-900 flex flex-col shadow-sm z-20">
        <div className="p-6 space-y-4">
          <div className="flex items-center justify-between">
            <h2 className="text-xs font-black uppercase tracking-[0.3em] text-zinc-400">Your Projects</h2>
            <Button 
              size="icon" 
              variant="ghost" 
              onClick={() => handleActionWithAuth(() => setIsCreateProjectModalOpen(true))}
          ...
          <div className="space-y-1">
            {!token ? (
              <div className="p-4 text-center space-y-3">
                <p className="text-xs italic text-zinc-400">Login to see projects</p>
                <Button variant="outline" size="sm" onClick={() => setIsLoginModalOpen(true)} className="rounded-full text-[10px] uppercase font-bold tracking-widest">
                  Login
                </Button>
              </div>
            ) : projectsLoading ? (
              Array(5).fill(0).map((_, i) => (
                <div key={i} className="h-12 w-full animate-pulse bg-zinc-50 dark:bg-zinc-800/50 rounded-xl mb-1" />
              ))
            ) : (
              projects.map((p: any) => (
                <button
                ...
                >
                  <div className="flex items-center gap-3">
                    <div className={`p-2 rounded-lg ${selectedProjectId === p.id ? 'bg-white/10 text-white dark:text-zinc-900' : 'bg-zinc-100 dark:bg-zinc-800'}`}>
                      <FaBriefcase className="text-xs" />
                    </div>
                    <span className="text-sm font-bold truncate max-w-[140px]">{p.name}</span>
                  </div>
                  <FaChevronRight className={`text-[10px] opacity-0 group-hover:opacity-100 transition-opacity ${selectedProjectId === p.id ? 'opacity-100' : ''}`} />
                </button>
              ))
            )}
          </div>
        ...
        <div className="max-w-5xl mx-auto py-12 px-8 space-y-12 pb-32">
          {activeProject ? (
            ...
            >
                ...
                <div className="space-y-2">
                  ...
                  <div className="flex flex-wrap gap-4 text-xs font-bold text-zinc-400 uppercase tracking-widest">
                    {activeProject.clientName && (
                      <span className="flex items-center gap-2"><FaUserAlt className="text-[10px]" /> {activeProject.clientName}</span>
                    )}
                    <span className="flex items-center gap-2"><SiFiverr className="text-[10px]" /> {activeProject.platform}</span>
                    {activeProject.orderLink && (
                      <a href={activeProject.orderLink} target="_blank" rel="noreferrer" className="flex items-center gap-2 text-instagram-blue hover:underline">
                  ...
                  >
                    {moderating ? (
                      ...
                      <>
                        ...
                        <span className="text-lg tracking-tight uppercase tracking-widest">Secure Message</span>
                      </>
                    )}
                  </Button>
                </div>
                ...
                {safeText && (
                  ...
                  >
                    <div className="flex items-center justify-between px-4">
                      <span className="text-xs font-black uppercase tracking-[0.3em] text-instagram-blue">Latest Secured Message</span>
                      <Button 
              ...
              <div className="space-y-6 pt-10 border-t border-zinc-100 dark:border-zinc-800">
                <div className="flex items-center gap-3 px-2">
                  ...
                  <h2 className="text-xs font-black uppercase tracking-[0.3em] text-zinc-400">Message History</h2>
                </div>
                
                <div className="space-y-4">
                  {combinedHistory.map((msg: any) => (
                    <div key={msg.id} className="group border border-zinc-100 dark:border-zinc-800 rounded-2xl p-6 bg-white dark:bg-zinc-900/50 hover:border-zinc-200 dark:hover:border-zinc-700 transition-all shadow-sm hover:shadow-md">
                      ...
                      <p className="text-xs text-muted-foreground mb-4 opacity-50 line-clamp-1 italic">Original: {msg.originalMsg}</p>
                  ...
                  {/* Infinite Scroll Trigger */}
                  {hasMore && (
                    <div ref={loadMoreRef} className="h-20 flex items-center justify-center pt-4">
                  ...
                  {combinedHistory.length === 0 && !historyLoading && (
                    <p className="text-sm italic text-zinc-300 px-2">No messages yet for this project.</p>
                  )}
                </div>
              </div>
            </motion.div>
            ...
            <div className="h-full flex flex-col items-center justify-center text-center space-y-6 opacity-30 pt-40">
              ...
              <div className="space-y-2">
                <h3 className="text-2xl font-black uppercase tracking-widest">Select a Project</h3>
                <p className="text-sm font-medium">Messages are tracked per project context for platform safety.</p>
              </div>
            </div>
          )}
        </div>
      ...
      <Dialog open={isLoginModalOpen} onOpenChange={setIsLoginModalOpen}>
        <DialogContent className="sm:max-w-md rounded-[2.5rem] border-none shadow-2xl">
          <DialogHeader className="space-y-4 text-center pb-4">
            ...
            <DialogTitle className="text-3xl font-black tracking-tighter">Freelancer Login</DialogTitle>
            <DialogDescription className="text-base font-medium">
              Protect your communication with AI. Sign in to track messages per project.
            </DialogDescription>
          </DialogHeader>
      ...
      <Dialog open={isCreateProjectModalOpen} onOpenChange={setIsCreateProjectModalOpen}>
        <DialogContent className="sm:max-w-lg rounded-[2.5rem] border-none shadow-2xl">
          <DialogHeader className="space-y-2">
            <DialogTitle className="text-2xl font-black tracking-tight">Create New Context</DialogTitle>
            <DialogDescription>
          ...
          <div className="grid gap-6 py-6">
            <div className="grid gap-2">
              <label className="text-[10px] font-black uppercase tracking-[0.2em] text-zinc-400 px-1">Project Name *</label>
              <Input
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="grid gap-2">
                <label className="text-[10px] font-black uppercase tracking-[0.2em] text-zinc-400 px-1">Client Name</label>
                <Input
                  placeholder="John"
              ...
              <div className="grid gap-2">
                <label className="text-[10px] font-black uppercase tracking-[0.2em] text-zinc-400 px-1">Platform</label>
                <Input
            ...
            <div className="grid gap-2">
              <label className="text-[10px] font-black uppercase tracking-[0.2em] text-zinc-400 px-1">Order URL</label>
              <Input
                placeholder="Fiverr Order Link..."
                value={projectForm.orderLink}
                onChange={(e) => setProjectForm({ ...projectForm, orderLink: e.target.value })}
                className="rounded-2xl h-14 bg-zinc-50 dark:bg-zinc-800 border-none px-6"
              />
            </div>
          </div>
          <div className="flex justify-end gap-3 pt-2">
            <Button variant="ghost" onClick={() => setIsCreateProjectModalOpen(false)} className="rounded-2xl h-12 px-8 font-black uppercase tracking-widest text-[10px]">
              Cancel
            </Button>
            <Button 
